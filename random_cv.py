...
def random_cv(cv_index, cv_year, roothpath, param_grid, num_random, model_name, device):
    # load data
    train_X = load_results(rootpath + 'train_X_pca_{}_forecast{}.pkl'.format(cv_year, cv_index + 1))
    valid_X = load_results(rootpath + 'val_X_pca_{}_forecast{}.pkl'.format(cv_year, cv_index + 1))
    train_y = load_results(rootpath + 'train_y_pca_{}_forecast{}.pkl'.format(cv_year, cv_index + 1))
    valid_y = load_results(rootpath + 'val_y_pca_{}_forecast{}.pkl'.format(cv_year, cv_index + 1))
    ...
    if model_name == 'EncoderFNN_AllSeq_AR_CI' or model_name == 'EncoderFNN_AllSeq_AR':
        ...
        train_y_ar = load_results(rootpath + 'train_y_pca_ar_forecast{}.pkl'.format(cv_index + 1))
        valid_y_ar = load_results(rootpath + 'train_y_pca_ar_forecast{}.pkl'.format(cv_index + 1))
    ...
    for i in range(num_random):
        ...
        if model_name == 'EncoderDecoder':
            ...
            curr_hidden_dim = hidden_dim[randint(0, len(hidden_dim) - 1)]
            curr_num_layer = num_layers[randint(0, len(num_layers) - 1)]
            curr_seq_len = decoder_len[randint(0, len(seq_len) - 1)]
            curr_threshold = threshold[randint(0, len(threshold) - 1)]
            curr_lr = lr[randint(0, len(lr) - 1)]
            curr_num_epochs = num_epochs[randint(0, len(num_epochs) - 1)]
            curr_linear_dim = linear_dim[randint(0, len(linear_dim) - 1)]
    ...
    save_results(rootpath + 'cv_results_test/cv_results_' + model_name + '_{}_{}.pkl'.format(cv_year, cv_index + 1), cv_results)


def best_hyperparameter(val_years, month_range, eval_metrics, model_name, roothpath):
...
rootpath = '/export/scratch/S2S/random_cv/'
...
num_random = 4
year = 2012
val_years = [2012, 2013, 2014, 2015]
month_range = [1]
Parallel(n_jobs=12)(delayed(random_cv)(cv_index, cv_year=year, roothpath=rootpath, param_grid=param_grid, num_random=num_random, model_name=model_name, device=device) for cv_index in range(1))
# best_hyperparameter(val_years=val_years, month_range=month_range, eval_metrics='cos', model_name=model_name, roothpath=rootpath)
